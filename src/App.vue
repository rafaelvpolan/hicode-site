...
.steps li { position: relative; display: flex; gap: var(--space-5); align-items: flex-start; background: var(--panel); background-image: linear-gradient(var(--grid-line) 1px, transparent 1px), linear-gradient(90deg, var(--grid-line) 1px, transparent 1px); background-size: var(--grid-cell) var(--grid-cell); border: 1px solid var(--bd); border-left: 3px solid var(--stage-color, var(--acc)); clip-path: polygon(var(--cut) 0, 100% 0, 100% calc(100% - var(--cut)), calc(100% - var(--cut)) 100%, 0 100%, 0 var(--cut)); padding: var(--space-7) var(--space-8); transition: transform var(--dur-soft) var(--ease-soft), border-color var(--dur-micro) ease; }
